...
public class ExaminationManager {
	...
	public static synchronized ExaminationManager getInstance(Context context) {
		if (instance == null) {
			synchronized (ExaminationManager.class) {
				if (instance == null) {
					instance = new ExaminationManager(context.getApplicationContext());
				}
			}
		}
		return instance;
	...
	public ExaminationInfo getExaminationInfoByIdcard(String idcard) {
		...
		where.append(" order by ").append(ExaminationInfoDao.Properties.CreateTime.columnName).append(" desc");
		List<ExaminationInfo> examinationInfoList = mExaminationInfoDao.queryRaw(where.toString(), idcard);
	...
	public void saveOrUpdateGeneralExamination(GeneralExamination generalExamination) {
	...
	public List<Question> getPhysiqueQuestions() {
		...
		XmlResourceParser parser = mContext.getResources().getXml(R.xml.test_physique);
		try {
	...
	public String getPrintTemplate(int printResId, String examinationNo) {
		...
		if (!TextUtils.isEmpty(result)) {
			...
			if (archiveInfo != null) {
				...
				personal_age = DateUtils.getAgeByBirthday(archiveInfo.getBirthday()) + "";
			}
			// 替换用户信息
			data.replace("{personal_name}", personal_name).replace("{personal_sex}", personal_sex).replace("{personal_age}", personal_age).replace("{test_num}", examinationNo);

			// 替换社区信息
			data.replace("{community_name}", mContext.getString(R.string.print_community_name)).replace("{community_address}", mContext.getString(R.string.print_community_address))
					.replace("{community_phone}", mContext.getString(R.string.print_community_phone)).replace("{community_service_hotline}", mContext.getString(R.string.print_community_service_hotline));
		}
		return data;
	}
}
