 ...
 * @version
 */
public class ElectrocardiogramAnalyzerFragment extends BaseQuickExaminationFragment {
	...
	private EditText heartRateEditText, PROfAVREditText, QTOfAVREditText, RvoltOfAVREditText, PvoltOfAVREditText, TvoltOfAVREditText, STvoltOfAVREditText;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View rootView = inflater.inflate(R.layout.fragment_quick_examination_electrocardiogram_analyzer, container, false);
		...
		showWaveformButton = (Button) rootView.findViewById(R.id.showWaveformButton);
		checkTimeEditText = (EditText) rootView.findViewById(R.id.checkTimeEditText);
		heartRateEditText = (EditText) rootView.findViewById(R.id.heartRateEditText);
		PROfAVREditText = (EditText) rootView.findViewById(R.id.PROfAVREditText);
		QTOfAVREditText = (EditText) rootView.findViewById(R.id.QTOfAVREditText);
		RvoltOfAVREditText = (EditText) rootView.findViewById(R.id.RvoltOfAVREditText);
		PvoltOfAVREditText = (EditText) rootView.findViewById(R.id.PvoltOfAVREditText);
		TvoltOfAVREditText = (EditText) rootView.findViewById(R.id.TvoltOfAVREditText);
		STvoltOfAVREditText = (EditText) rootView.findViewById(R.id.STvoltOfAVREditText);
		conclusionEditText = (EditText) rootView.findViewById(R.id.conclusionEditText);
		electrocardiogram = null;
		
		// 使用当前日期
		checkTimeEditText.setText(DateUtils.formatDate(new Date(), "yyyy-MM-dd"));
		return rootView;
	}
	...
	private class ReadDataTask extends AsyncTask<Void, Void, Electrocardiogram> {
		...
		protected Electrocardiogram doInBackground(Void... params) {
			if (electrocardiogramManager.connectDevice()) {
				Electrocardiogram electrocardiogram = electrocardiogramManager.readData();
				electrocardiogramManager.closeDevice();
		...
		protected void onPostExecute(Electrocardiogram result) {
			...
			if (result != null) {
				...
				ToastUtils.showToast(activity, electrocardiogramManager.getTipsInfo());
			}
	...
	public void onConclusion() {
		if (electrocardiogram != null) {
			...
			//heartRateStatus： 000(0)-心率正常，001(1)-心率稍慢，010(2)-心律过慢，011(3)-心律稍快，100(4)-心律过快，101(5)-导连脱落
			//心率过慢
	...
	public String getPrintData(String examinationNo) {
		String printStr = ExaminationManager.getInstance(getActivity()).getPrintTemplate(R.raw.print_electrocardiogram_template, examinationNo);
		// 替换相关数据
		printStr=printStr.replace("{check_time}",checkTimeEditText.getText().toString())// 检查时间
						 ...
						 .replace("{Rvolt_of_AVR}", RvoltOfAVREditText.getText().toString())// 平均R波电压
						 .replace("{Pvolt_of_AVR}", PvoltOfAVREditText.getText().toString())// 平均P波电压
						 .replace("{Tvolt_of_AVR}", TvoltOfAVREditText.getText().toString())// 平均T波电压
						 .replace("{STvolt_of_AVR}", STvoltOfAVREditText.getText().toString())// ST段平均电压
						 .replace("{conclusion}", conclusionEditText.getText().toString());// 心电图结果
		return printStr;
	}
	...
	public void getSaveData(ExaminationInfo examinationInfo) {
		...
		try {
			...
			ecgAnalysis.put("checkTime", checkTimeEditText.getText().toString());
			ecgAnalysis.put("pulseRate", heartRateEditText.getText().toString());
			ecgAnalysis.put("PROfAVR", PROfAVREditText.getText().toString());
			ecgAnalysis.put("QTOfAVR", QTOfAVREditText.getText().toString());
			ecgAnalysis.put("RvoltOfAVR", RvoltOfAVREditText.getText().toString());
			ecgAnalysis.put("PvoltOfAVR", PvoltOfAVREditText.getText().toString());
			ecgAnalysis.put("TvoltOfAVR", TvoltOfAVREditText.getText().toString());
			ecgAnalysis.put("STvoltOfAVR", STvoltOfAVREditText.getText().toString());
			ecgAnalysis.put("conclusion", conclusionEditText.getText().toString());
