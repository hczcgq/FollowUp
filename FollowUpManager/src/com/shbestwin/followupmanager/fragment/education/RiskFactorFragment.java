...
public class RiskFactorFragment extends BaseFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View rootView = inflater.inflate(R.layout.fragment_education_risk_factor, container, false);
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		// 1.16-1.42mmol/L
		if (bloodHdl < 1.16) {
			addItem("高密度脂蛋白偏低", R.string.jkjy_fxys_tips_high_density_lipoprotein_low);
		} else if (bloodHdl > 1.42) {
			addItem("高密度脂蛋白偏高", R.string.jkjy_fxys_tips_high_density_lipoprotein_over);
		}
		...
		// 正常参考范围：2.1-3.1。
		if (bloodLdl < 2.1) {
			addItem("低密度脂蛋白偏低", R.string.jkjy_fxys_tips_low_density_lipoprotein_low);
		} else if (bloodLdl > 3.1) {
			addItem("低密度脂蛋白偏高", R.string.jkjy_fxys_tips_low_density_lipoprotein_over);
		} 
	...
	private void addItem(String title, int contentResId) {
		ViewGroup view = (ViewGroup) View.inflate(getActivity(), R.layout.view_risk_factor_item, null);
		((CardHeaderLayout) view.getChildAt(0)).setTitleText(title);
		((TextView) view.getChildAt(1)).setText(Html.fromHtml(getString(contentResId)));
